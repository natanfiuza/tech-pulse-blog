<template>
  <AdminLayout>
    <div class="mx-auto max-w-7xl">
      <h1 class="mb-6 font-headline text-2xl font-extrabold text-on-surface md:text-3xl">Editar Post</h1>
      ...
      <form @submit.prevent="submit('publicado')" class="flex flex-col items-start gap-8 lg:flex-row">
        ...
        <div class="w-full shrink-0 space-y-6 lg:w-80">
          ...
          <div class="rounded-xl border border-outline-variant/20 bg-surface-container-low p-5 shadow-2xl">
            ...
            <ImageDropzone v-model:model_value="form.image" :image_src="imagem_atual" :error="form.errors.image" />
          </div>
            ...
            <div>
              ...
              >
                ...
                <option
                  ...
                  :key="categoria.id"
                  :value="categoria.id"
                  :style="{ fontWeight: categoria.nivel === 0 ? 'bold' : 'normal' }"
                >
...
import { url_da_imagem } from "@/helpers.js";
...
// Imagem atual do post, normalizada para a rota post/image/{filename}
const imagem_atual = computed(() => {
    if (!props.post?.image && !props.post?.uuid) {
        return "";
    }
    return url_da_imagem(props.post);
});
...
const categorias_planas = computed(() => {
    ...
    // Garante que a categoria atual do post apareça mesmo fora da árvore carregada
    if (form.category_id && !resultado.some((c) => c.id === form.category_id)) {
        resultado.push({
            id: form.category_id,
            name: props.post?.category?.name ?? `Categoria #${form.category_id}`,
            nivel: 0,
        });
    }
    return resultado;
...
function formatar_datetime_local(valor) {
    if (!valor) {
        return "";
    }
