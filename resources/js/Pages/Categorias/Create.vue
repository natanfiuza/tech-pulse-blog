    ...
    <div>
        ...
        <form @submit.prevent="submit">
            ...
            <div class="mb-3">
                ...
                <div v-if="form.errors.scope" class="invalid-feedback">{{ form.errors.scope }}</div>
            </div>

            <div class="mb-3">
                <label for="possible_contents" class="form-label">Possíveis Conteúdos:</label>
                <textarea id="possible_contents" v-model="form.possible_contents" class="form-control"></textarea>
                <div v-if="form.errors.possible_contents" class="invalid-feedback">{{ form.errors.possible_contents }}
                </div>
            </div>

            <div class="mb-3">
                <label for="post_suggestions" class="form-label">Sugestões de Postagens:</label>
                <textarea id="post_suggestions" v-model="form.post_suggestions" class="form-control"></textarea>
                <div v-if="form.errors.post_suggestions" class="invalid-feedback">{{ form.errors.post_suggestions }}
                </div>
            </div>
            ...
            <button type="submit" class="btn btn-primary" :disabled="form.processing">Criar</button>
        </form>
    </div>
</template>
