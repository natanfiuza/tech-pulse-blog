...
<head>
    ...
    <title>{{ $strings['mail_cancel_subject'] ?? 'Link para Cancelar Inscrição' }}</title>
</head>
                        ...
                        <tr>
                            <td style="padding:32px 24px;">
                                <h1 style="font-size:20px;color:#f8fafc;margin:0 0 16px;font-weight:800;">
                                    {{ $strings['mail_cancel_subject'] ?? 'Cancelar Inscrição' }}
                                </h1>
                                <p style="font-size:15px;color:#cbd5e1;line-height:1.6;margin:0 0 24px;">
                                    {{ $strings['mail_cancel_intro'] ?? 'Recebemos uma solicitação para cancelar o envio da Newsletter TechPulse. Clique no botão abaixo para confirmar o descadastro.' }}
                                </p>
                                <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                                    <tr>
                                        <td align="center" style="padding:12px 0 28px;">
                                            <a href="{{ $url }}" style="display:inline-block;background-color:#ef4444;color:#ffffff;padding:14px 28px;border-radius:8px;text-decoration:none;font-size:15px;font-weight:700;letter-spacing:0.3px;">
                                                {{ $strings['mail_cancel_button'] ?? 'Cancelar Inscrição' }}
                                            </a>
                                        </td>
                                    </tr>
                                </table>
