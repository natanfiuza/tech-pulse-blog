...
- Files you may touch: `resources/js/Layouts/AdminLayout.vue`, `resources/js/Layouts/DashboardResponsivo.vue` (only if used by admin pages), `resources/js/Components/Admin/*`, `resources/js/Pages/Admin/**`, `resources/css/tailwind.css` (custom classes only), `resources/js/Components/Admin/MarkdownEditor.vue` (styling only).
...
4. **PostsCreate / PostsEdit** — prototype form: title input, EasyMDE markdown editor (kept), right column with Publicar / Salvar Rascunho buttons, featured image dropzone (`Components/Admin/ImageDropzone.vue`), category select (from `categorias` tree), tags input (`Components/Admin/TagInput.vue`, suggestions from `hashtags_existentes`), scheduling `datetime-local` input.
